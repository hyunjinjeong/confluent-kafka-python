...
## How to build for `manylinux`

1. Build Dockerfile

Set `NAME` and `VERSION` properly.

```shell
docker build -t {NAME}:{TAG} .
```

2. Run Dockerfile w/ volume mount.

```shell
docker run -it -v $(pwd)/dist:/dist {NAME}:{TAG}
# In container
cp * /dist
```

3. Check `./dist` folder and upload wheel files using `twine`.

```shell
cd dist
twine upload {WHEEL_NAME}.whl
```
...
_(requires librdkafka + dependencies to be installed separately)_:

    $ pip install --no-binary :all: confluent-kafka

For source install, see _Prerequisites_ below.
